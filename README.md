# [books-manager](http://joshdmiller.github.com/ng-boilerplate) [![Build Status](https://api.travis-ci.org/ngbp/ngbp.png?branch=v0.3.2-release)](https://travis-ci.org/ngbp/ngbp)
...
`books-manager` is designed to make life easy by providing a basic framework
...
books-manager also comes prepackaged with the most popular design frameworks
...
The principal goal of `books-manager` is to set projects up for long-term
success.  So `books-manager` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `books-manager`.

But `books-manager` is not an example of an AngularJS app: this is a
...
getting `books-manager` up and running. Though `books-manager` is really simple
...
`books-manager` uses [Grunt](http://gruntjs.org) as its build system, so
...
There are many Bower packages used by `books-manager`, like Twitter Bootstrap
...
Technically, `books-manager` is now ready to go.
...
`books-manager`. What follows in this section is a quick introduction to the
...
`books-manager` also includes [Live Reload](http://livereload.com/), so you no
...
default with `books-manager`) you need to tell Live Reload to allow it. Go to
